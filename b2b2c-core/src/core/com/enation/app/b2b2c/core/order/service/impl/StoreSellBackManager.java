...
public class StoreSellBackManager implements IStoreSellBackManager {
	...
	public Page list(int page, int pageSize, Integer store_id,Integer status,Map map) {
		...
		StringBuffer sql = new  StringBuffer("select * from es_sellback_list s inner join es_order o on o.order_id=s.orderid");
		String condition=" WHERE ";
		if(store_id!=null){
			sql.append(condition+" s.store_id = "+store_id);
			condition=" AND ";
		}
		if(status!=null){
			sql.append(condition+"s.tradestatus="+status);
			condition=" AND ";
		}
		if(start_time!=null){
			sql.append(condition+"s.regtime>"+start_time);
			condition=" AND ";
		}
		if(end_time!=null){
			sql.append(condition+"s.regtime<"+end_time);
			condition=" AND ";
		}
		if(map.size()>0){
