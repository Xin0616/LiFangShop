...
//		System.out.println(sql.toString());
	 ...
	 */
	@Override
	public List<Map> averYear_Amount(Integer status, int year,String store_id) {
		String condition_sql = createSqlByYear(1, year+"");
		String sql =  "select count(0) as t_num,round(SUM(need_pay_money)/COUNT(DISTINCT member_id),2)  AS aver ,SUM(need_pay_money) as t_money, case "+ condition_sql +" as month  from es_order o where 1=1 ";
		String storeWhere="";
		if(store_id != null && !"0".equals(store_id)){
			storeWhere = " AND o.store_id = "+store_id;
		}
	 ...
	 */
	@Override
	public List<Map> averMonth_Amount(Integer status, int year,int month,String store_id) {
		String condition_sql = createSql(1, year,month);
		String sql =  "select count(0) as t_num,round(SUM(need_pay_money)/COUNT(DISTINCT member_id),2)  AS aver ,SUM(need_pay_money) as t_money, case "+ condition_sql +" as month  from es_order o where 1=1";
		String storeWhere="";
		if(store_id != null && !"0".equals(store_id)){
