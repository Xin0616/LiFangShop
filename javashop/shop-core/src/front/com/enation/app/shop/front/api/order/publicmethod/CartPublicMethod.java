...
public class CartPublicMethod {
	...
	public JsonResult addGroupCart(Product product, int num, int showCartData, Integer group_id) {
		...
		if(product != null) {
			try {
				...
				Cart tempCart = cartManager.getCartByGroupEndDate(group_id, sessionid);
				if(tempCart == null){
				
						throw new RuntimeException("抱歉！团购时间已经结束。");
				...
				Double  price= cartManager.getPrice(group_id);
				cart.setPrice(price);
