...
public class MemberCommentManager  implements IMemberCommentManager{
	...
	public Page getGoodsComments(int goods_id, int page, int pageSize, int type, Integer grade) {
		String sql = "SELECT m.lv_id,m.sex,m.uname,m.face,l.name as levelname, m.nickname,c.* FROM es_member_comment" + " c LEFT JOIN es_member" + " m ON c.member_id=m.member_id LEFT JOIN es_member_lv" + " l ON m.lv_id=l.lv_id " +
