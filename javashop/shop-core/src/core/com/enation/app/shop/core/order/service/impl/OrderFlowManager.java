...
//			throw new RuntimeException("创建订单失败，购物车为空");
	...
	public void confirmOrder(Integer orderId) {
		...
		this.daoSupport.execute("update es_order set status=?  where order_id=?", OrderStatus.ORDER_CHANGE ,orderId);
	...
	private void saveGoodsItem(List<CartItem> itemList, Order order) {
		...
		for (int i = 0; i < itemList.size(); i++) {
			...
			Integer a=cartItem.getGroup_id();
			orderItem.setGroup_id(cartItem.getGroup_id());
