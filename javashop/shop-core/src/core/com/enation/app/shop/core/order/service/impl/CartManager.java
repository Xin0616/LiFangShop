...
public class CartManager implements ICartManager {
	...
	@Override
	public Cart getCartByGroupId(int group_id, String sessionid){
		...
		return (Cart)this.daoSupport.queryForObject("SELECT * FROM es_cart WHERE group_id=? AND (session_id=? or member_id=?)", Cart.class, group_id,sessionid,memberid);
	}
	
	@Override
	public Cart getCartByGroupEndDate(int group_id, String sessionid){
		...
		return (Cart)this.daoSupport.queryForObject("SELECT * FROM  es_groupbuying WHERE id=?  AND end_date>unix_timestamp()" , Cart.class, group_id);
	}
	/*
	 * (non-Javadoc)
	...
	public List<CartItem> listGoods(String sessionid) {
		...
		sql.append(" c.is_check,c.group_id,c.cart_id,g.cat_id as catid,g.goods_id,g.thumbnail,c.name ,  p.sn, p.specs  ,g.mktprice,g.unit,g.point,p.product_id,c.price,c.cart_id as cart_id,c.num as num,c.itemtype,c.addon,c.weight,c.activity_id from es_cart c, es_product p,es_goods g ");
	...
	public List<CartItem> selectListGoods(String sessionid) {
		...
		sql.append(" c.is_check,c.group_id,g.cat_id as catid,g.goods_id,g.thumbnail,c.name ,  p.sn, p.specs  ,g.mktprice,g.unit,g.point,p.product_id,c.price,c.cart_id as cart_id,c.num as num,c.itemtype,c.addon,c.weight,c.activity_id  from es_cart c, es_product p,es_goods g ");
...
//		HttpCacheManager.sessionChange();
	...
	public OrderPrice countPrice(List<CartItem> cartItemList, Integer shippingid,String regionid) {
		...
		Map map = getStorePrice(cartItemList);
		Double df = getDeliveryFee(map);
		//配送费
		orderPrice.setDeliveryFeePrice(df);
		...
		//订单总金额:商品金额+配送费
		orderTotal = CurrencyUtil.add(orderTotal, df);
	...
	public Map getStorePrice(List<CartItem> cartList){
		...
		for (CartItem cartItem : cartList) {
			if(cartItem.getIs_check() != null && cartItem.getIs_check() == 1){
	...
	@Override
	public Double getPrice(Integer group_id) {
		// TODO Auto-generated method stub
		StringBuffer sql = new StringBuffer("select buying_money from es_groupbuying where id=?  ");
		return this.daoSupport.queryForDouble(sql.toString(), group_id);
	}
