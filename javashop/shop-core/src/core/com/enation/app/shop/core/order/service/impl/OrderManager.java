...
public class OrderManager implements IOrderManager {
	...
	@Log(type=LogType.ORDER,detail="修改订单ID为${orderid}的订单金额")
	public void savePrice(double price, int orderid) {
		//Order order = this.get(orderid);
		
		double delivery_amount = deliveryFeeManager.getPriceByFull(price);
		
		//应付金额 = 订单金额 - 优惠券金额
		double needPayMoney = CurrencyUtil.add(price, delivery_amount);
		
		this.daoSupport.execute("update es_order set order_amount=?, need_pay_money=?, delivery_amount=? where order_id=?", price, needPayMoney, delivery_amount, orderid);
...
//			this.orderPluginBundle.onCanel(order);
	...
	@Override
	public Page listByStatusAndApartment(Integer status, Integer apartment_id, int pageNo, int pageSize) {
		String sql = "select o.*,"
				+ " s.attr as store_addr, s.tel as store_mobile,"
				+ " m.mobile as member_mobile, m.nickname as member_nickname"
				+ " from (select * from es_order where parent_id is not null and status = " + status + ") o"
				+ " left join es_store s on o.store_id = s.store_id"
				+ " left join es_member m on o.member_id = m.member_id"
				+ " where m.apartment_id = " + apartment_id;
	...
	public void updateStatus(String ids, Integer status) {
		...
		for (String id : str) {
			Order order = get(Integer.parseInt(id));
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public void updateGroupOrderMoney(Integer orderId, Double price) {
		// TODO Auto-generated method stub
		Order order = this.get(orderId);
		if(order.getIs_group() == null || order.getIs_group().equals("0")) {
			return;
		}
		Double advanceCharge = this.getAdvanceCharge(orderId);
		Double restCharge = order.getNeed_pay_money() - advanceCharge;
		if(advanceCharge.equals(price)) {
			String sql = "update es_order set advance_charge = " + price + " where order_id = " + orderId;
			this.daoSupport.execute(sql);
			List<OrderItem> list = this.daoSupport.queryForList("select * from es_order_items where order_id = " + orderId, OrderItem.class);
			for(OrderItem orderItem : list) {
				String updateCountSql = "update es_groupbuying set number = number + " + orderItem.getNum() + " where id = " + orderItem.getGroup_id();
				this.daoSupport.execute(updateCountSql);
			}
		}
		if(restCharge.equals(price)) {
			String sql = "update es_order set rest_charge = " + price + " where order_id = " + orderId;
			this.daoSupport.execute(sql);
		}
	}
	
	@Override
	public Double getAdvanceCharge(Integer orderId) {
		String sql = "select sum(a.earnest) from es_groupbuying a where a.id in (select group_id from es_order_items b where b.order_id = "+ orderId + ")";
		return this.daoSupport.queryForDouble(sql);
	}

	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public void updateStatusCourier(String ids, Integer status) {
		Member member = UserConext.getCurrentMember();
		String[] str = ids.split(",");
		for (String id : str) {
			Order order = get(Integer.parseInt(id));
			order.setStatus(status);
			order.setCourier_id(member.getMember_id());
			this.daoSupport.update("es_order", order, "order_id = " + order.getOrder_id());
		}
