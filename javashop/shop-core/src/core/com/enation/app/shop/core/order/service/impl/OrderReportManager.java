...
public class OrderReportManager implements IOrderReportManager {
	...
	public Page listReturned(int pageNo, int pageSize, String order,String type) {
		String sql = "select s.*,o.is_group from es_sellback_list s inner join es_order o on s.orderid=o.order_id where 1=1";
		if(type!=null){
			sql +=" and s.type="+type;
		}
		/**
		...
		if("b2b2c".equals(EopSetting.PRODUCT) ){
			 sql +=" and s.store_id="+ShopApp.self_storeid;
		}
		if (!StringUtil.isEmpty(order)) {
			sql += " and s.tradestatus = "+order+"";
		}
