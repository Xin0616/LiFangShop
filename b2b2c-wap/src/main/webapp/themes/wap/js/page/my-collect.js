...
$(function () {
    ...
    $("#store").on("tap",function(){
    	location.href = "store.html?type=store&page=1"
    })
    $("#goods").on("tap",function(){
    	location.href = "my-collect.html?type=goods&page=1"
    })
    function bindEvents() {
                    ...
                    success: function (res) {
                        if(res.result == 1){
                            module.message.success('取消成功！', function () {
                                location.replace(ctx + '/member/my-collect.html?type=goods&page=1');
                            });
                        }else {
            ...
            function _ajax() {
                $.ajax({
                    ...
                    success: function (res) {
                        if(res.result == 1){
                            module.message.success('取消成功！', function () {
                                location.replace(ctx + '/member/store.html?type=store&page=1');
                            });
                        }else {
                            module.message.error(res.message);
                        }
