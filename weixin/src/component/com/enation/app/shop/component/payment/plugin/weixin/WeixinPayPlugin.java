...
//		Double money = order.getNeedPayMoney();
		...
		if(orderDetail.getIs_group() == null || orderDetail.getIs_group().equals("0")) {
			money = order.getNeedPayMoney();
		} else {
			if(orderDetail.getAdvance_charge() == null || orderDetail.getAdvance_charge().equals(0d)) {
				money = orderManager.getAdvanceCharge(order.getOrder_id());
			}
			if(orderDetail.getRest_charge() == null || orderDetail.getRest_charge().equals(0d)) {
				money = order.getNeedPayMoney() - orderManager.getAdvanceCharge(order.getOrder_id());
			}
		}
